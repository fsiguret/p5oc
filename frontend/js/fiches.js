...
let display = new DisplayProducts();
//set id.
...
request.getProductAsync()
    .then(product => {
        let teddy = new Teddy(product.id, product.imageUrl, product.price, product.description, product.name, product.colors);
        display.printProduct(teddy);
